## What this changes

<!-- One or two sentences. What is different after this merges? -->

Closes #

## How to check it

<!-- How should a reviewer verify this works? -->

## Checklist

- [ ] One issue per pull request
- [ ] Commits signed off (`git commit -s`)
- [ ] Works in **both** English and Nepali
- [ ] Keyboard operable, visible focus, sufficient contrast
- [ ] Uses design tokens — no hard-coded colours or spacing
- [ ] Tests added or updated for behaviour changes
- [ ] No new dependency (or agreed in the issue first)
- [ ] **No secrets, no real personal data anywhere in the diff**

## Screenshots

<!-- For any visible change. Both languages if the text differs. -->
